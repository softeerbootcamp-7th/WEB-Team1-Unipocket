...
public class TravelQueryRepository {
	...
	public Optional<TravelQueryResponse> findById(Long id) {
		List<TravelQueryResponse> result =
				em.createQuery(
								"SELECT new com.genesis.unipocket.travel.query.persistence.response.TravelQueryResponse("
										+ "t.id, t.accountBookId, t.travelPlaceName, t.startDate, t.endDate, t.imageKey) "
										+ "FROM Travel t WHERE t.id = :id",
	...
	public List<TravelQueryResponse> findAllByAccountBookId(Long accountBookId) {
		return em.createQuery(
						"SELECT new com.genesis.unipocket.travel.query.persistence.response.TravelQueryResponse("
								+ "t.id, t.accountBookId, t.travelPlaceName, t.startDate, t.endDate, t.imageKey) "
								+ "FROM Travel t WHERE t.accountBookId = :accountBookId",
						TravelQueryResponse.class)
	...
	public List<WidgetOrderDto> findAllByTravelId(Long travelId) {
		return em.createQuery(
						"SELECT new com.genesis.unipocket.travel.query.persistence.response.WidgetQueryResponse("
								+ "w.widgetType, w.widgetOrder) "
								+ "FROM TravelWidget w WHERE w.travel.id = :travelId "
