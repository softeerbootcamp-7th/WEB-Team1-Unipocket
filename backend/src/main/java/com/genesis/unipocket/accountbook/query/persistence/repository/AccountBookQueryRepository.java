...
public class AccountBookQueryRepository {
    ...
    public Optional<AccountBookQueryResponse> findById(Long id) {
        List<AccountBookQueryResponse> result = em.createQuery(
                "SELECT new"
                        + " com.genesis.unipocket.accountbook.query.persistence.response.AccountBookQueryResponse("
                        + " a.id, a.title, a.localCountryCode, a.baseCountryCode, a.startDate, a.endDate)"
                        + " FROM AccountBookEntity a WHERE a.id = :id",
    ...
    public Optional<AccountBookDetailResponse> findDetailById(Long id) {
        ...
        // needed
        List<AccountBookDetailResponse> result = em.createQuery(
                "SELECT new"
                        + " com.genesis.unipocket.accountbook.query.persistence.response.AccountBookDetailResponse("
                        + " a.id, a.title, a.localCountryCode, a.baseCountryCode, a.budget, null, a.startDate,"
                        + " a.endDate)"
                        + " FROM AccountBookEntity a WHERE a.id = :id",
