...
public class UserQueryRepository {
    ...
    public Optional<UserQueryResponse> findById(UUID id) {
        List<UserQueryResponse> result = em.createQuery(
                "SELECT new com.genesis.unipocket.user.query.persistence.response.UserQueryResponse("
                        + "u.id, u.email, u.name, u.profileImgUrl, u.role, u.status) "
    ...
    public List<UserCardQueryResponse> findAllCardsByUserId(UUID userId) {
        return em.createQuery(
                "SELECT new com.genesis.unipocket.user.query.persistence.response.UserCardQueryResponse("
                        + "c.userCardId, c.nickName, c.cardNumber, c.cardCompany) "
                        + "FROM UserCardEntity c WHERE c.user.id = :userId",
