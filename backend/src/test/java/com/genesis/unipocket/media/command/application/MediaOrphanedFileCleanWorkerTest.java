...
class MediaOrphanedFileCleanWorkerTest {
	...
	@Mock private MediaObjectStoragePort mediaObjectStoragePort;
	...
	void clean_deletesOnlyOrphanedKeys() {
		MediaOrphanedFileCleanWorker worker =
				new MediaOrphanedFileCleanWorker(
						mediaObjectStoragePort, List.of(provider1, provider2));
		when(mediaObjectStoragePort.listAllKeys()).thenReturn(List.of("a", "b", "c"));
		when(provider1.getUsedPaths()).thenReturn(Set.of("a"));
		when(provider2.getUsedPaths()).thenReturn(Set.of("c"));

		worker.clean();

		verify(mediaObjectStoragePort).deleteObjects(List.of("b"));
	}
	...
	void clean_whenNoOrphans_doNotDelete() {
		MediaOrphanedFileCleanWorker worker =
				new MediaOrphanedFileCleanWorker(
						mediaObjectStoragePort, List.of(provider1, provider2));
		when(mediaObjectStoragePort.listAllKeys()).thenReturn(List.of("a", "b"));
		when(provider1.getUsedPaths()).thenReturn(Set.of("a", "b"));
		when(provider2.getUsedPaths()).thenReturn(Set.of());

		worker.clean();

		verify(mediaObjectStoragePort, never()).deleteObjects(anyList());
	}
