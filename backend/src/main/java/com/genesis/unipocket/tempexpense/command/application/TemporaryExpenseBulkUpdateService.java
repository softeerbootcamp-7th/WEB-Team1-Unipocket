...
public class TemporaryExpenseBulkUpdateService {
	...
	public TemporaryExpenseMetaBulkUpdateResult updateByFile(
		...
		Map<Long, TemporaryExpense> scopedExpenseById =
				temporaryExpenseRepository
						.findScopedByIds(requestedIds, tempExpenseMetaId, fileId)
						.stream()
						.collect(
								Collectors.toMap(
	...
	public void deleteByFile(
		...
		TemporaryExpense target =
				temporaryExpenseRepository
						...
						.orElse(null);
		if (target == null) {
			if (temporaryExpenseRepository.existsById(tempExpenseId)) {
				throw new BusinessException(ErrorCode.TEMP_EXPENSE_SCOPE_MISMATCH);
			}
			throw new BusinessException(ErrorCode.TEMP_EXPENSE_NOT_FOUND);
		}
	...
	private TemporaryExpenseResult updateTemporaryExpense(
