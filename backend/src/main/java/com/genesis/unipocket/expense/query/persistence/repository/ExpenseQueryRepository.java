		...
		return em
				.createQuery(
						"SELECT new"
							...
							+ " e.exchangeInfo.calculatedBaseCurrencyCode END, CASE WHEN"
							+ " e.exchangeInfo.baseCurrencyAmount IS NOT NULL THEN"
	...
	public Page<ExpenseQueryRow> findExpenses(
		...
		String selectJpql =
				"SELECT new"
					...
					+ " e.exchangeInfo.localCurrencyAmount, e.occurredAt, e.updatedAt,"
					+ " e.merchant.displayMerchantName, e.approvalNumber, e.userCardId,"
					+ " e.expenseSourceInfo.expenseSource, e.expenseSourceInfo.fileLink, e.memo,"
					+ " e.cardNumber) FROM ExpenseEntity e "
						+ whereClause
						+ " "
						+ orderByClause;
